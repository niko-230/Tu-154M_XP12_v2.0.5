...
defineProperty("baro_press_inhg", globalPropertyf("sim/weather/barometer_sealevel_inhg"))        -- XP10/11: давление inHg
...
local T = {
	...
	n2_c_q    = 0.000018,
	n2_M_rot  = 0.18,    -- масса ротора КВД Д-30
	...
	-- [SCALE] Таблица масштабирования симового N2 → показание прибора (%)
	-- Д-30КУ-154 реальные значения:
	--   idle земля: ~62%   | посадка с механизацией: ~80%  | взлёт: ~98-100%
	-- XP12 симовый N2 для этого самолёта даёт завышенные значения в верхней части:
	--   sim ~62  → прибор 62  (idle совпадает)
	--   sim ~90  → прибор 80  (посадочный режим)
	--   sim ~105 → прибор 98  (взлётный)
	--   sim >107 → прибор 100 (максимум шкалы)
	n2_scale = {
		{  0,   0 },
		{ 20,  20 },
		{ 62,  62 },  -- idle земля
		{ 72,  70 },  -- малый газ полётный
		{ 90,  80 },  -- посадочный режим с механизацией
		{ 98,  88 },  -- крейсер
		{105,  98 },  -- взлётный
		{107, 100 },  -- максимум шкалы
		{200, 100 },
	},
	-- [SCALE] Таблица масштабирования симового N1 → показание прибора (%)
	-- Д-30КУ-154 реальные значения:
	--   idle: ~30-35%  | посадка: ~55-65%  | взлёт: ~85-90%  | максимум шкалы: 95%
	n1_scale = {
		{  0,   0 },
		{ 20,  20 },
		{ 35,  33 },  -- idle
...
local Tdr = {
	...
	-- bleed air: отбор воздуха на кондиционирование и противообледенение
	bleed_air = globalPropertyf("sim/cockpit/misc/bleed_air_mode"),
	anti_ice1 = globalPropertyf("sim/cockpit/switches/anti_ice_inlet_heat_per_enigne[0]"),
	anti_ice2 = globalPropertyf("sim/cockpit/switches/anti_ice_inlet_heat_per_enigne[1]"),
	anti_ice3 = globalPropertyf("sim/cockpit/switches/anti_ice_inlet_heat_per_enigne[2]"),
	-- crossbleed: кран перекрёстного питания (кросс-питание при запуске)
	crossbleed = globalPropertyf("sim/cockpit/pneumatics/engine_crossfeed_on"),
}
...
if MASTER then
	...
	local disp_n2_1 = math.min(100, interpolate(T.n2_scale, T.ang1 + T.n2_adv1 + surge1_osc))
	...
	local disp_n2_2 = math.min(100, interpolate(T.n2_scale, T.ang2 + T.n2_adv2 + surge2_osc))
	...
	local disp_n2_3 = math.min(100, interpolate(T.n2_scale, T.ang3 + T.n2_adv3 + surge3_osc))
	...
	local disp_n1_1 = math.min(95, interpolate(T.n1_scale, T.ang1b))
	...
	local disp_n1_2 = math.min(95, interpolate(T.n1_scale, T.ang2b))
	...
	local disp_n1_3 = math.min(95, interpolate(T.n1_scale, T.ang3b))
