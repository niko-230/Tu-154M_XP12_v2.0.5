...
local T = {
	...
	n2_scale = {
		{  0,    0   },
		{ 20,   18   },
		{ 67,   58   },  -- ниже МГ — отбросная зона
		{ 70,   60.5 },  -- малый газ земля: sim~70 → РЛЭ 59.5-61.5%
		{ 78,   65   },  -- ~0,6 номин. нижний
		{ 84,   70   },  -- между 0,6 и 0,7
		{ 89,   76   },  -- ~0,7 номин.
		{ 93,   83   },  -- ~0,9 номин.
		{ 96,   89   },  -- ~номинальный
...
if MASTER then
	...
	local idle_jitter1 = bool2int(flame1>0 and rpm_1 < 80) * (math.random()-0.5) * 0.4
	...
	local idle_jitter2 = bool2int(flame2>0 and rpm_2 < 80) * (math.random()-0.5) * 0.4
	...
	local idle_jitter3 = bool2int(flame3>0 and rpm_3 < 80) * (math.random()-0.5) * 0.4
	...
	local idle_jitter1b = bool2int(flame1>0 and T.N2need1 < 60) * (math.random()-0.5) * 0.35
	...
	local idle_jitter2b = bool2int(flame2>0 and T.N2need2 < 60) * (math.random()-0.5) * 0.35
	...
	local idle_jitter3b = bool2int(flame3>0 and T.N2need3 < 60) * (math.random()-0.5) * 0.35
