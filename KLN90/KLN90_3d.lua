...
defineProperty("kln_knob_out", globalPropertyi("sim/custom/switchers/kln_knob_out")) -- вытягивание ручки КЛН
defineProperty("kln_power_knob", globalPropertyi("sim/custom/switchers/kln_power_knob")) -- вдавливание ручки питания КЛН
...
------------------------------------------------------------------------------------
-- Tu154 ABSU
defineProperty("kln_course", globalPropertyf("sim/custom/kln90/kln_course")) -- курс ЛЗП от КЛН
defineProperty("kln_dev", globalPropertyf("sim/custom/kln90/kln_dev")) -- отклонение от ЛЗП, мили
defineProperty("kln_flag", globalPropertyi("sim/custom/kln90/kln_flag")) -- флаг курса КЛН. 0 = флаг
--------------------------------------
--------------------------------------
...
defineProperty("BAROin", globalPropertyf("sim/weather/barometer_current_inhg2"))
...
--defineProperty("HSIOBS", globalPropertyf("sim/custom/gauges/compas/pkp_obs_L")) -- курс полета на ПКПdefineProperty("HSIOBS", globalPropertyf("sim/custom/gauges/compas/pkp_obs_set_L"))  -- set the course
...
defineProperty("PSIin", globalPropertyf("sim/flightmodel/position/mag_psi"))
...
function WPTpage(types, mode, subpage)
				...
				while test[1]["types"] ~= types do
					...
					elseif lpage == 7 then
						...
						elseif lsubpage[7] == 3 then
							...
							-- X-Plane 12: apt.dat moved to Global Scenery/Global Airports/
							-- Try XP12 path first, then XP11 fallbacks
							filename = "Global Scenery/Global Airports/Earth nav data/apt.dat"
							file = io.open(filename, "r")
							if file == nil then
								filename = "Resources/default data/default apt dat/Earth nav data/apt.dat"
								file = io.open(filename, "r")
							end
							if file == nil then
							...
							-- Detect apt.dat version (XP12 uses version 1200, XP11 uses 1100/1050)
							local apt_version = 1100
							if file then
								local hdr1 = file:read("*line") -- "I" or "A"
								local hdr2 = file:read("*line") -- version line, e.g. "1200 Generated by..."
								if hdr2 then
									local ver = tonumber(string.match(hdr2, "^(%d+)"))
									if ver and ver >= 1200 then apt_version = 1200 end
								end
								-- rewind by reopening
								file:close()
								file = io.open(filename, "r")
							...
							while true do
								...
								elseif string.sub(line, 1, 1) == "5" then
									...
									-- XP12 apt.dat v1200: frequencies stored as integer Hz (e.g. 121900000)
									-- XP11 apt.dat: frequencies stored as MHz*100 integer (e.g. 12190)
									if apt_version >= 1200 then
										local freq_hz = tonumber(string.match(line, "^%d+%s+(%d+)"))
										if freq_hz then
											-- convert Hz -> MHz string "NNN.NN"
											local mhz = freq_hz / 1000000
											local mhz_int = math.floor(mhz)
											local mhz_frac = math.floor((mhz - mhz_int) * 100 + 0.5)
											file2:write(airport .. "|5|" .. station .. string.format("%03d", mhz_int) .. "." .. string.format("%02d", mhz_frac) .. "\n")
										end
									else
							...
							file:close()
							file2:close()
							-- Try Custom Data (user-installed nav cycle) first, then default
							filename = "Custom Data/earth_nav.dat"
							file = io.open(filename, "r")
							if file == nil then
	...
	p1.visible = false -- permanently disabled: this is the classic 2D popup, replaced by the SASL3 KLN90B popup; noClose=true means it can't be dismissed from the UI, so it's force-disabled here instead
